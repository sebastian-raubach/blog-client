        ...
        <v-row justify="center" align="center" class="text-center">
          <v-col cols="12" md="9" lg="7">
            <h1 class="text-h2 text-sm-h1 font-weight-black text-white mb-4 text-uppercase">
              <v-icon :icon="mdiImageFilterHdr" class="me-2" /> Berichte zum Berg
            </h1>

            <v-chip :text="hill.hillName" v-if="hill" :color="hillConfigs[hill.hillType].color" variant="elevated" :prepend-icon="hillConfigs[hill.hillType].icon" size="large" />
          </v-col>
        </v-row>
      </div>
    </v-img>
...
<script setup lang="ts">
  ...
  import type { ViewHills, ViewPosts } from '@/plugins/types/blog'
  import { mdiImageFilterHdr } from '@mdi/js'

  const route = useRoute('/hill/[id]')
  ...
  onMounted(() => {
    apiGetHillById(+route.params.id, result => {
      hill.value = result
    })
  })
</script>
