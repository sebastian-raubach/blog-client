  ...
  <v-dialog v-model="dialog" :width="`min(90vw, ${internalWidth || 400}px)`">
    <v-card>
      ...
      <v-card-actions class="pt-0">
        ...
        <v-btn :color="internalOkVariant" text @click="emitOk">{{ internalOkTitle }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
<script lang="ts" setup>
  ...
  function show (params: any) {
    ...
    internalOkTitle.value = params.okTitle || 'OK'
    internalCancelTitle.value = params.cancelTitle || 'Abbrechen'
